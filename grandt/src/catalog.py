...
class DefeCatalog(Catalog):
	...
	def query_get_all_players_to_show(self):
		query = 'SELECT name, position, category FROM players'
		return query

	def query_get_all_teams_to_show(self):
		query = 'SELECT * FROM users_teams'
		return query

	def get_name_from_players_ids(self, players_ids):
		query = 'SELECT name FROM players WHERE player_id IN ({ids})'
		return query.format(ids=','.join(str(_id) for _id in players_ids))

	def query_get_all_teams_points(self):
		query = 'SELECT p.dni, t.user_name, t.team_name, p.round, p.points  FROM points_of_team_by_round p, users_teams t WHERE p.dni = t.dni'
		return query

	def query_get_player_points_by_round(self, player_id):
		query = 'SELECT round, total_points FROM history h WHERE {player_id} = h.player_id'
		return query.format(player_id = player_id )

	def query_get_player_info_from_name(self, player_name):
